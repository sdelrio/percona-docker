...
#get list of IP from swarm dns discovery
ip_list=$(dig A tasks.$SWARM_SERVICE +short)
export join_address=$(echo "$ip_list" |awk -v myip="$ipaddr" 'BEGIN {ORS=","} myip !~ $0 {print $1}'| sed 's/,$//')
cluster_join=${join_address}
